...
public class TcpListener extends AbstractSocketListener<TcpAddress>
{
    ...
    {
        return address(super.getZAddress());
    }

    protected String address(Address.IZAddress address)
    {
        int port = super.getFd().socket().getLocalPort();
    ...
    public boolean setAddress(final String addr)
    {
        ...
        return this.setZAddress(new TcpAddress(addr, options.ipv6));
    }
    ...
    {
        //  Convert the textual address into address structure.
        return this.setZAddress(new TcpAddress(addr));
    }
