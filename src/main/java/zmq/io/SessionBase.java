...
public class SessionBase extends Own implements Pipe.IPipeEvents, IPollEvents {
    ...
    // This set is added to with pipes we are disconnecting, but haven't yet
    // completed
    ...
    public int handleErrorReason(String reason) {
        int rc = -1;
        if (reason.length() == 3
                && reason.charAt(1) == '0'
                && reason.charAt(2) == '0'
            ...
            try {
                ...
                getSocket().eventHandshakeFailedAuth(getEndpoint(), statusCode);
                rc = 0;
            } catch (NumberFormatException e) {
                getSocket().eventHandshakeFailedProtocol(getEndpoint(),
    ...
    public void engineError(boolean handshaked, ErrorReason reason) {
        ...
        if (pipe != null) {
            ...
            // Only send disconnect message if socket was accepted and handshake was
            // completed
