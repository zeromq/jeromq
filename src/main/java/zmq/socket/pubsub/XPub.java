...
public class XPub extends SocketBase {
    ...
    // Subscriptions will not bed added automatically, only after calling set option
    // with ZMQ_SUBSCRIBE or ZMQ_UNSUBSCRIBE
    private boolean manual;

    // Last pipe that sent subscription message, only used if xpub is on manual
    private Pipe lastPipe;

    // Pipes that sent subscriptions messages that have not yet been processed, only
    // used if xpub is on manual
    ...
    public boolean checkZMQ(int option) {
        return option == ZMQ.ZMQ_XPUB_VERBOSE || option == ZMQ.ZMQ_XPUB_VERBOSER
    ...
    protected void xpipeTerminated(Pipe pipe) {
        if (manual) {
            ...
            subscriptions.rm(pipe, (p, d, s, self) -> {
            }, this);
        } else {
