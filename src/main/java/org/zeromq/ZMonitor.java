...
public class ZMonitor implements Closeable {
    ...
    public static final class ZEvent {
        // Enum.values return a different array on each call, so keep it to avoid
        // useless allocations
        ...
        /**
         * The numerical code of the event. (useful in case of unrecognized event, in
         * which case type is ALL)
         */
    ...
    public enum ProtocolCode {
        ...
        /**
         * Find the {@link ProtocolCode} associated with the numerical error code send
         * with <code>eventHandshakeXXX</code>.
         * 
     ...
     * 
     * @param wait true to block until next event is available, false to immediately
     *             return with null if there is no event.
     ...
     * Gets the next event, blocking for it until available if requested.
     * 
     * @param timeout the time in milliseconds to wait for a message before
     *                returning null, -1 to block.
     * @return the next event or null if there is currently none after the specified
     *         timeout.
     */
