 ...
 * <li>Pause: <b><i>lets the socket queues accumulate messages according to
 * their types</i></b>
 ...
 * <li>{@link #restart(ZMsg) Hot Restart}: <i>User-defined behavior with custom
 * messages</i>
 * <li>{@link #configure(ZMsg) Configure}: <i>User-defined behavior with custom
 * messages</i>
 ...
 * </ul>
 * All the non-custom commands can be performed in asynchronous or synchronous
 * mode.
 * <br>
 * <li>Proxy mechanism ensured by pluggable pumps
 * <ul>
 * <li>with built-in low-level {@link org.zeromq.ZProxy.ZmqPump} (zmq.ZMQ):
 * useful for performances
 * <li>with built-in high-level {@link org.zeromq.ZProxy.ZPump} (ZeroMQ): useful
 * for {@link org.zeromq.ZProxy.ZPump.Transformer message transformation}, lower
 * performances
 * <li>with your own-custom proxy pump implementing a {@link Pump 1-method
 * interface}
 * </ul>
 * </ul>
 * <p>
 * <br>
 * You can have all the above non-customizable features in about these lines of
 * code:
 * 
 * <pre>
 * {@code
 ...
 * </pre>
 * 
 * Once created, the proxy is not started. You have to perform first a start
 * command on it.
 * This choice was made because it is easier for a user to start it with one
 * line of code than for the code to internally handle
 * different possible starting states (after all, someone may want the proxy
 * started but paused at first or configured in a specific way?)
 ...
 * </pre>
 *
 * A {@link #command(Command, boolean) programmatic interface} with enums is
 * also available.
 *
 *
 */
...
public class ZProxy {
    ...
    // Contract for socket creation and customizable configuration in proxy
    // threading.
    public interface Proxy {
        /**
         * Creates and initializes (bind, options ...) the socket for the given plug in
         * the proxy.
         * The proxy will close them afterwards, and the context as well if not provided
         * in the constructor.
         * There is no need to keep a reference on the created socket or the context
         * given in parameter.
         *
         * @param ctx   the context used for initialization.
         * @param place the position for the future created socket in the proxy.
         * @param args  the optional array of arguments that has been passed at the
         *              creation of the ZProxy.
         ...
         * @param args   the optional array of arguments that has been passed at the
         *               creation of the ZProxy.
         * @return true to perform a cold restart instead, false to do nothing. All the
         *         results will be collected from calls for all plugs.
         ...
         *
         * Note: you need to send one (1) mandatory custom response message with the
         * pipe before the end of this call.
         *
     ...
     * {@link #newZProxy(ZContext, String, org.zeromq.ZProxy.Proxy, String, Object...)
     * low-level one}.
     *
     * @param ctx        the context used for the proxy.
     *                   Possibly null, in this case a new context will be created
     *                   and automatically destroyed afterwards.
     * @param name       the name of the proxy. Possibly null.
     * @param selector   the creator of the selector used for the internal polling.
     *                   Not null.
     * @param sockets    the sockets creator of the proxy. Not null.
     * @param motdelafin the final word used to mark the end of the proxy. Null to
     *                   disable this mechanism.
     * @param args       an optional array of arguments that will be passed at the
     *                   creation.
     *
     * @return the created proxy.
     * @deprecated use
     *             {@link #newZProxy(ZContext, String, Proxy, String, Object...)}
     *             instead.
     */
    @Deprecated
     ...
     *
     * @return the created proxy.
     * @deprecated use {@link #newProxy(ZContext, String, Proxy, String, Object...)}
     *             instead.
     */
     ...
     * Starts the proxy.
     *
     * @param sync true to read the status in synchronous way, false for
     *             asynchronous mode
     ...
     * Does not works with commands {@link #CONFIG CONFIG} and {@link #RESTART
     * RESTART}.
     *
     ...
     * Sends a command message to the proxy actor.
     * Can be useful for programmatic interfaces.
     * Does not works with commands {@link Command#CONFIG CONFIG} and
     * {@link Command#RESTART RESTART}.
     *
     ...
     * Sends a command message to the proxy actor.
     * Can be useful for programmatic interfaces.
     * Works only with commands {@link Command#CONFIG CONFIG} and
     * {@link Command#RESTART RESTART}.
     *
     * @param command the command to execute.
     ...
     *
     * @param hot null to make a cold restart (closing then re-creation of the
     *            sockets)
    ...
    // defines the very first time where no command changing the state has been
    // issued
     ...
     *                   If null, a new context will be created and closed at the
     *                   stop of the operation.
     *                   <b>If not null, it is the responsibility of the call to
     *                   close it.</b>
     * @param name       the name of the proxy (used in threads naming).
     * @param selector   the creator of the selector used for the proxy.
     ...
     *                   creation.
     * @deprecated use
     *             {@link #ZProxy(ZContext, String, Proxy, Pump, String, Object...)}
     *             instead.
     */
    @Deprecated
    ...
    /**
     * Creates a new named proxy. A new context will be created and closed at the
     * stop of the operation.
     *
     * @param name       the name of the proxy (used in threads naming).
     * @param sockets    the creator of the sockets of the proxy.
    ...
    public interface Pump {
        /**
         * Transfers a message from one source to one destination, with an optional
         * capture.
         *
         * @param src         the plug of the source socket
         * @param source      the socket where to receive the message from.
         * @param capture     the optional sockets where to send the message to.
         *                    Possibly null.
         * @param dst         the plug of the destination socket
         * @param destination the socket where to send the message to.
         *
         * @return false in case of error or interruption, true if successfully
    ...
    private static final class ProxyActor extends ZActor.SimpleActor {
        ...
        public boolean start(ZPoller poller) {
        ...
        private boolean pause(ZPoller poller, boolean pause) {
            ...
            if (pause) {
                ...
                // TODO why not a mechanism for eventually flushing the sockets during the
                // pause?
            } else {
    ...
    public static class ZPump implements Pump {
        ...
        public interface Transformer {
             ...
             * If you return back another message than the one given in input, then this one
             * has to be destroyed by you.
             * 
    ...
    private static final class ZmqPump implements Pump {
        // transfers each message as a whole by sending each packet received to the
        // capture socket
        @Override
        public boolean flow(Plug splug, Socket source, Socket capture, Plug dplug, Socket destination) {
            boolean rc;
